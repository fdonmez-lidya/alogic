  ...
  "The Checker should" - {
      ...
      "rejecting them in root entities" - {
        for (word <- List("read", "write")) {
          word in {
            ...
            inside(node) {
              case Decl(_, DescEntity(_, List(EntDecl(main)))) =>
                inside(main) {
                  case Decl(_, DescFunc(_, _, _, List(stmt))) =>
    ...
    "reject output slices on ports" - {
      "with no flow control" in {
        ...
        inside(tree) {
          case Decl(_, DescEntity(_, List(decl))) =>
            inside(decl) {
              case EntDecl(Decl(_, DescOut(_, fc, st, _))) =>
      ...
      "declarations with disallowed type" - {
        for {
          ...
          decl in {
            s"""|struct a {
                |  ${decl};
                |}""".stripMargin.asTree[Decl] rewrite checker should matchPattern {
              case Decl(_, DescRecord(Nil)) =>
            }
      ...
      for (entity <- List("fsm", "network")) {
        entity in {
          val tree = s"""|${entity} a {
                         ...
                         | verbatim verilog {}
                         |}""".stripMargin.asTree[Decl]
          tree rewrite checker shouldBe a[Decl]
      ...
      "rejecting them outside loops" - {
        for (word <- List("break", "continue")) {
          word in {
            ...
            inside(node) {
              case Decl(_, DescEntity(_, List(main))) =>
                inside(main) {
                  case EntDecl(Decl(_, DescFunc(_, _, _, List(_, stmt)))) =>
                    stmt shouldBe StmtError()
                }
            }
