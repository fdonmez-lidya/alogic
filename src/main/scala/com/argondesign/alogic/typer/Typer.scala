...
final class Typer(implicit cc: CompilerContext) extends TreeTransformer {
  ...
  private def hasUnknown(node: TreeLike): Boolean = node.children exists {
    case child: Tree => child.hasTpe && child.tpe.isUnknown
    case TypeError   => true
    case child: Type => child.children exists hasUnknown
    case _           => unreachable
  }
  ...
  // Utility that will issue the compiler error 'msg' at the 'mark' node,
  ...
  override def enter(tree: Tree): Unit = {
    ...
    tree match {
      case decl @ Decl(Sym(symbol, idxs), desc) =>
        assert(idxs.isEmpty)

        if (symbol.kind.isPacked && symbol.kind.underlying != TypeVoid && symbol.kind.width < 1) {
          error(tree, s"'${symbol.name}' declared with width ${symbol.kind.width}")
        }
        ...
        // Allow unary ticks in the initializer
        desc.initializer foreach { init =>
          pushContextWidth(init, symbol.kind)
        }
  ...
  override def transform(tree: Tree): Tree = {
    ...
    tree match {
      ...
      case Decl(ref, DescFunc(FuncVariant.Ctrl, _, _, body)) =>
        if (body.isEmpty) {
        ...
        if (field.isEmpty) {
          expr.tpe match {
            case TypeEntity(symbol, _) =>
              error(
                tree,
                s"No port named '$sel' on instance '${expr.toSource}' of entity '${symbol.name}'")
            case kind =>
              error(tree,
