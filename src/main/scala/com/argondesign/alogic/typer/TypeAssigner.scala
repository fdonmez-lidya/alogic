...
object TypeAssigner {
  ...
  private def kind(tree: ExprSelect)(implicit cc: CompilerContext) = tree.expr.tpe match {
...
//      println(tree, kind); println(kind.symbol.decl.toSource); kind(tree.selector).get.kind
//    case kind: CompoundType => println(tree, kind); kind(tree.selector).get.kind
    case kind: CompoundType => kind(tree.selector).get.kind
    case _                  =>
      // TODO: this should be unreachable but some backend passes,
      // (SplitStructsA in particular) create some ill-typed temporary nodes
      TypeError
  }
  ...
  private def assign(tree: Tree)(kind: => Type): tree.type = {
    ...
    def hasError(node: TreeLike): Boolean = {
