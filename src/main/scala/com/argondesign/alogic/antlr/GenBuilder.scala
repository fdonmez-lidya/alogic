...
object GenBuilder extends BaseBuilder[GenContext, Gen] {
  ...
  def apply(ctx: GenContext)(implicit cc: CompilerContext): Gen = {
    object GenItemVisitor extends AlogicScalarVisitor[Tree] {
      ...
      override def visitGenItemDecl(ctx: GenItemDeclContext): Tree = DeclBuilder(ctx.decl)
    ...
    object GinitVisitor extends AlogicScalarVisitor[Decl] {
      override def visitGinit(ctx: GinitContext): Decl = {
        val ident = IdentBuilder(ctx.IDENTIFIER)
        val spec = ExprBuilder(ctx.expr(0))
        val init = ExprBuilder(ctx.expr(1))
        val loc = ctx.loc.copy(point = ctx.IDENTIFIER.getStartIndex)
        val desc = DescGen(spec, init) withLoc loc
    ...
    object GenVisitor extends AlogicScalarVisitor[Gen] {
      ...
      override def visitGenRange(ctx: GenRangeContext): Gen = {
        val decl = {
          val ident = IdentBuilder(ctx.IDENTIFIER)
          val spec = ExprBuilder(ctx.expr(0))
          val loc = ctx.IDENTIFIER.loc.copy(start = spec.loc.start)
          val init = ExprNum(false, 0) withLoc loc
          val desc = DescGen(spec, init) withLoc loc
          Decl(ident, desc) withLoc loc
        }
