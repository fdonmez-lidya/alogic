...
object EntBuilder extends BaseBuilder[EntContext, Ent] {
  def apply(ctx: EntContext)(implicit cc: CompilerContext): Ent = {
    object Visitor extends AlogicScalarVisitor[Ent] {
      override def visitEntDecl(ctx: EntDeclContext): Ent = {
        val decl = DeclBuilder(ctx.decl) match {
          case func @ Decl(_, desc: DescFunc) =>
            func.copy(desc = desc.copy(variant = FuncVariant.Ctrl) withLoc desc.loc) withLoc func.loc
          case other => other
        }
        EntDecl(decl) withLoc ctx.loc
      }
