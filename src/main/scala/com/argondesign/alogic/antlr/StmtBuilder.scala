...
object StmtBuilder extends BaseBuilder[ParserRuleContext, Stmt] {
  ...
  def apply(ctx: ParserRuleContext)(implicit cc: CompilerContext): Stmt = {
    object Visitor extends AlogicScalarVisitor[Stmt] { self =>
      override def visitStmtDecl(ctx: StmtDeclContext): Stmt =
      ...
      override def visitLoopInitDecl(ctx: LoopInitDeclContext): Stmt = {
        ...
        val desc = DescVar(spec, Some(init)) withLoc loc
        val decl = Decl(ident, desc) withLoc loc
        StmtDecl(decl) withLoc loc
      }
