...
trait TypeOps extends TypePrintOps { this: Type =>
  ...
  final def isChoice: Boolean = this.isInstanceOf[TypeChoice]
  ...
  final def asChoice: TypeChoice = this.asInstanceOf[TypeChoice]
  ...
  // Create a Desc node describing this Type
  def desc(initOpt: Option[Expr] = None): Desc = {
    ...
    this match {
      case k: TypeFund        => DescVar(spec(k), initOpt)
      case TypeIn(k, fc)      => DescIn(spec(k), fc)
      case TypeOut(k, fc, st) => DescOut(spec(k), fc, st, initOpt)
      case TypeConst(k)       => DescConst(spec(k), initOpt.get)
      case TypeArray(e, size) => DescArray(spec(e), ExprNum(false, size))
//      case TypeSram(e, size, st) => DescSram(spec(e), ExprNum(false, size), st)
      case TypeStack(e, size) => DescStack(spec(e), ExprNum(false, size))
  ...
  def desc: Desc = this match {
    case kind: TypeEntity => DescInstance(ExprSym(kind.symbol))
    case kind: TypeChoice => DescChoice(kind.symbols map ExprSym)
    case _                => desc(None)
  }

  def desc(init: Expr): Desc = desc(Some(init))
