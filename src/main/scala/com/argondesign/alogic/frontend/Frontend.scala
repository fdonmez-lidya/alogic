...
class Frontend(
  ...
  private[this] val rootDecls = mutable.ListBuffer[Decl]()
  ...
  private[this] def childNames(trees: List[Tree]): List[String] = trees flatMap {
    case decl @ Decl(_, _: DescEntity)  => List(decl.name)
    case EntDecl(decl)                  => childNames(List(decl))
    ...
    lazy val future = {
      ...
      val astFuture = preprocessedFuture map { source =>
        parseIt(source, name) tap { root =>
          ...
          val decl = root.decls filter { _.name == name } match {
      ...
      val childAstsFuture = {
        ...
        val instantiatedEntityNamesFuture = astFuture map { root =>
          def loop(decl: Decl): List[String] = decl match {
            case Decl(Ident(n, _), desc: DescEntity) =>
              val localNames = n :: childNames(desc.body)

              def gatherExternalNames(trees: List[Tree]): List[String] = trees flatMap {
                case Decl(_, DescInstance(spec)) =>
                  spec match {
                ...
                case EntDecl(decl)           => gatherExternalNames(List(decl))
              ...
              gatherExternalNames(desc.body) ::: (desc.decls flatMap loop)
            case _ => Nil
          }
          root.decls flatMap loop
        }
  ...
  // and global decls
  def apply(topLevelNames: List[String]): (List[Root], List[Decl]) = {
    ...
    for {
      Root(body) <- treeSet
      RizDecl(Decl(ref, desc)) <- body
      if desc.isInstanceOf[DescEntity]
      ident: Ident = ref.asInstanceOf[Ident]
      if topLevelNames contains ident.name
    } {
      ident.attr("toplevel") = Expr(1) withLoc ident.loc
    }

    // Return the trees and global decls
    (treeSet.toList, rootDecls.toList)
  }
}
